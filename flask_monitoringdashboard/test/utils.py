...
def add_fake_data():
    ...
    from flask_monitoringdashboard.database import session_scope, Request, Endpoint, Outlier, TestsGrouped
    from flask_monitoringdashboard import config

    # Add requests
    with session_scope() as db_session:
        for i in range(len(EXECUTION_TIMES)):
            call = Request(id=REQUEST_IDS[i], endpoint_id=ENDPOINT_ID, duration=EXECUTION_TIMES[i], version_requested=config.version,
