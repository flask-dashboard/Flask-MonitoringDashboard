    ...
    $scope.generateReport = function () {
        ...
        if ($scope.activeSection === "commits") {
            ...
            promise = $http.post(`api/reporting/make_report/intervals`, {
                interval: {
                    to: parseInt(
                        `${$scope.intervals.comparison.to.getTime() / 1000}`,
                    ),
                    from: parseInt(
                        `${$scope.intervals.comparison.from.getTime() / 1000}`,
                    ),
                },
                baseline_interval: {
                    to: parseInt(
                        `${$scope.intervals.baseline.to.getTime() / 1000}`,
                    ),
                    from: parseInt(
                        `${$scope.intervals.baseline.from.getTime() / 1000}`,
                    ),
                },
            });
        }
