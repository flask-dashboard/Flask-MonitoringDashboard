...
from flask_monitoringdashboard.database.count_group import count_requests_per_day, \
    get_value
...
def get_status_code_distribution(db_session, endpoint_id):
    ...
    return {status_code: frequency / total_count for (status_code, frequency) in
            status_code_counts}
...
def get_status_code_frequencies(db_session, endpoint_id, criterion):
    ...
    """
    status_code_counts = db_session.query(Request.status_code,
                                          func.count(Request.status_code)) \
        .filter(Request.endpoint_id == endpoint_id, Request.status_code.isnot(None),
                criterion) \
        .group_by(Request.status_code)
...
def get_status_code_frequencies_in_interval(db_session, endpoint_id, criterion):
...
def get_hourly_load(db_session, endpoint_id, start_date, end_date):
    ...
    hours = ['0{}:00'.format(h) for h in range(0, 10)] + ['{}:00'.format(h) for h in
                                                          range(10, 24)]
    ...
    for time, count in get_num_requests(db_session, endpoint_id, start_datetime,
                                        end_datetime):
    ...
    return {
        'days': [
            (start_date + datetime.timedelta(days=i)).strftime('%Y-%m-%d') for i in
            range(numdays)
        ],
