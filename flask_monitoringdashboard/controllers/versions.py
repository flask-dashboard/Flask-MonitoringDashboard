...
def get_2d_version_data(db_session, endpoint_id, versions, column_data, column):
    """
    ...
    values = get_two_columns_grouped(db_session, column, Request.endpoint_id == endpoint_id)
...
def get_version_user_data(db_session, endpoint_id, versions, users):
    return get_2d_version_data(db_session, endpoint_id, versions, users, Request.group_by)


def get_version_ip_data(db_session, endpoint_id, versions, ips):
    return get_2d_version_data(db_session, endpoint_id, versions, ips, Request.ip)


def get_multi_version_data(db_session, endpoints, versions):
    """
    :param db_session: session for the database
