...
class TestStackLine(unittest.TestCase):
    ...
    def test_add_stackline(self):
        with session_scope() as db_session:
            ...
            add_stack_line(
                db_session, request_id=1, position=0, indent=1, duration=1, code_line="code"
            )
    ...
    def test_get_profiled_requests(self):
        with session_scope() as db_session:
            self.assertEqual(
                get_profiled_requests(db_session, endpoint_id=ENDPOINT_ID, offset=0, per_page=10),
                [],
            )
            add_stack_line(
                db_session,
    ...
    def test_get_grouped_profiled_requests(self):
        with session_scope() as db_session:
            ...
            self.assertNotEqual(
                get_grouped_profiled_requests(db_session, endpoint_id=ENDPOINT_ID), []
            )
